...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_rustdesk() {
            if s.contains("HelpDesk")
                && !name.starts_with("upgrade_helpdesk_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("HelpDesk") {
                    s = s.replace("HelpDesk", &app_name);
                } else {
                    // https://github.com/rustdesk/rustdesk-server-pro/issues/845
                    // If app_name contains "HelpDesk" (e.g., "HelpDesk-Admin"), we need to avoid
                    // replacing "HelpDesk" within the already-substituted app_name, which would
                    // cause duplication like "HelpDesk-Admin" -> "HelpDesk-Admin-Admin".
                    //
